...
function renderChart(data) {
    ...
    const labels = Object.keys(districtStats).sort();
    const dataAktif = labels.map(l => districtStats[l].aktif);
    const dataTutup = labels.map(l => districtStats[l].tutup);

    if (ppsChartInstance) {
        ppsChartInstance.destroy();
    }

    ppsChartInstance = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [
